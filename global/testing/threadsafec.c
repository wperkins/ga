...
int main(int argc, char * argv[])
{
    ...
    // Threads grab data from global array and copy them into a local buffer 
    // and verify that data is correct.
    if (id == 0) {
      GA_Zero(g_count);
      ok = 1;
      GA_Zero(g_time);
    ...
    //END NON-BLOCK WRITE TEST
    
    #pragma omp barrier
    
    //START NON-BLOCK READ TEST
    ...
    //END NON-BLOCK READ TEST
    
    #pragma omp barrier
    
    //START NON-BLOCK ACC TEST
    if (id == 0) {
      GA_Zero(g_count);
      if (me==0) {
    ...
    //END NON-BLOCK ACC TEST
    
    #pragma omp barrier
    
    //START NON-BLOCK RAN TEST
    if (id == 0) {
      /* Sync all processors*/
      NGA_Sync();
